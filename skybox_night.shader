...
Shader "Shader Forge/skybox_night" {
    Properties {
        ...
        _star_texture ("star_texture", 2D) = "white" {}
        _star_intensity ("star_intensity", Range(0, 10)) = 3.564103
    }
    SubShader {
        Tags {
        ...
        Pass {
            ...
            uniform sampler2D _star_texture; uniform float4 _star_texture_ST;
            uniform float _star_intensity;
            struct VertexInput {
...
////// Lighting:
////// Emissive:
                float4 _star_texture_var = tex2D(_star_texture,TRANSFORM_TEX(i.uv0, _star_texture));
                float3 node_790 = (_SkyColor.rgb*(_star_texture_var.rgb*_star_intensity));
                ...
                float node_9430 = 1.0;
                float3 emissive = (lerp(node_790,node_790,pow((1.0 - max(0,dot(viewDirection,float3(0,-1,0)))),8.0))+(_LightColor0.rgb*pow(saturate((node_9430 + ( (max(0,dot((-1*lightDirection),viewDirection)) - node_1476) * (0.0 - node_9430) ) / ((1.0 - (node_9367*node_9367)) - node_1476))),5.0)*_SunIntensity));
